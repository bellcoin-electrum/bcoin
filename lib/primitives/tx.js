...
const bellcoin_yespower = require('bellcoin-yespower');
...
TX.prototype.hash = function hash(enc) {
  ...
  if (!h) {
    h = bellcoin_yespower.hash(this.toNormal());
    if (!this.mutable)
      this._hash = h;
  }
...
TX.prototype.witnessHash = function witnessHash(enc) {
  ...
  if (!hash) {
    hash = bellcoin_yespower.hash(this.toRaw());
    if (!this.mutable)
      this._whash = hash;
  }
...
TX.prototype.signatureHashV0 = function signatureHashV0(index, prev, type) {
  ...
  return bellcoin_yespower.hash(bw.render());
};
...
TX.prototype.signatureHashV1 = function signatureHashV1(index, prev, value, type) {
  ...
  if (!(type & hashType.ANYONECANPAY)) {
    if (this._hashPrevouts) {
      ...
      prevouts = bellcoin_yespower.hash(bw.render());
  ...
  if (!(type & hashType.ANYONECANPAY)
    ...
    if (this._hashSequence) {
      ...
      sequences = bellcoin_yespower.hash(bw.render());
  ...
  if ((type & 0x1f) !== hashType.SINGLE
    ...
    if (this._hashOutputs) {
      ...
      outputs = bellcoin_yespower.hash(bw.render());
    ...
    if (index < this.outputs.length) {
      const output = this.outputs[index];
      outputs = bellcoin_yespower.hash(output.toRaw());
    }
  }
