...
*/
  ...
  if (prev.height > LwmaAveragingWindow) {
     return await this.Lwmaretarget(time, prev);
  }
  // Back 2 weeks
...
/**
 * Calculate difficulty using LWMA formula.
 * @param {Number} time - Next block timestamp.
 * @param {ChainEntry} prev - Previous entry.
 ...
 *
 */
Chain.prototype.Lwmaretarget = async function Lwmatarget(time, prev) {
  const pow = this.network.pow;

  const T = 60;
  const N = LwmaAveragingWindow;
  const k = N * (N + 1) * T / 2;
  assert(prev.height > N);

  let t = 0, j = 0, solvetime;
  let sum_target;

  for (let i = prev.height - (N + 1); i <= prev.height; i++) {
  	const block = await this.getAncestor(prev, i);
  	const block_prev = await this.getAncestor(prev, i - 1);
  	solvetime = block->time - block_prev->time;
  	solvetime = Math.max(- 6 * T, Math.min(solvetime, 6 * T));
  	j++;
  	t += solvetime * j;
  	
  	const ptarget = consensus.fromCompact(prev.bits);
    const sum_target =+ ptarget / (k * N);
  }
  if (t < k / 10) {
  	t = k / 10;
  }

  const target = t * sum_target;
